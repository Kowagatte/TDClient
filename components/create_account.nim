...
import gdnim, godotapi / [engine, node, line_edit, button, label, scene_tree, link_button]
import connection

gdnim CreateAccount of Node:
  ...
  method sendCreation() =
    if self.passwordEnter.text != self.confirmPasswordEnter.text:
      ...
      self.connection.sendData(construct(createAccountPacket))
